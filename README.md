...
<head>
    ...
    <title>Magic Christmas v1.2 - Optimized</title>
...
<body>
    <!-- UI Layer -->
    <div id="ui-layer">
        <div id="status" class="status-badge">🎄 Giáng Sinh An Lành 🎄</div>
        <div class="guide">
            <b>🖐 XÒE TAY:</b> Bung quà & xoay ảnh &nbsp;|&nbsp;
            <b>👌 CHỤM NGÓN:</b> Phóng to ảnh<br>
            <b>✊ NẮM TAY:</b> Thu về cây thông &nbsp;|&nbsp;
            <b>🫶 2 TAY TẠO TIM:</b> Hiện "I Love You"<br>
            <i style="color:#FFA500;">Di chuyển tay trái/phải để xoay chọn ảnh</i>
        </div>
        <button id="btnStart" onclick="startSystem()">🎅 BẮT ĐẦU</button>
    </div>
    <!-- Hand position indicator -->
    <div id="hand-indicator" style="display:none;">
        <span>Vị trí tay</span>
        <div class="bar">
            <div class="bar-fill" id="hand-bar"></div>
        </div>
    </div>
    <div id="copyright">© by vandiep - v1.2</div>
    <div id="canvas-container"></div>
    <video class="input_video" style="display:none"></video>
    <canvas id="camera-preview"></canvas>
    <script>
        // ==========================================
        // 1. RESOURCES CONFIG (Optimized)
        // ==========================================
        ...
        // ==========================================
        // 2. OPTIMIZED CONFIG (giảm particles)
        // ==========================================
        const CONFIG = {
            goldCount: 1200,     // Giảm từ 2000 → 1200
            redCount: 200,       // Giảm từ 300 → 200
            giftCount: 100,      // Giảm từ 150 → 100
            explodeRadius: 65,
            photoOrbitRadius: 25,
            treeHeight: 70,
            treeBaseRadius: 35,
            // NEW: Faster rotation
            rotationSpeed: 8.0,   // Tăng từ 4.0 → 8.0 (2x faster)
            lerpSpeed: 0.15       // Tăng từ 0.1 → 0.15
        };
        ...
        let selectedIndex = 0;
        let handX = 0.5;
        let smoothHandX = 0.5;  // NEW: Smoothed hand position
        // ==========================================
        // 3. THREE.JS SYSTEM
        // ==========================================
        function init3D() {
        ...
        function createDecorations() {
            ...
            lCtx.fillText("iu eimmm ❤️", 512, 130);
        ...
        function updateParticleGroup(group, type, targetState, speed, handRotY, time) {
            ...
            if (targetState === 'TREE') {
                ...
                // FASTER rotation following hand - key improvement!
        ...
        function animate() {
            ...
            const speed = 0.08;
            // SMOOTH hand tracking - key improvement!
            smoothHandX += (handX - smoothHandX) * 0.15;
            const handRotY = (smoothHandX - 0.5) * CONFIG.rotationSpeed;
            // Update hand indicator bar
            const handBar = document.getElementById('hand-bar');
            if (handBar) handBar.style.width = (smoothHandX * 100) + '%';
            ...
            if (state === 'TREE') {
                ...
                photoMeshes.forEach((mesh, i) => {
                    ...
                    mesh.position.lerp(new THREE.Vector3(x, y, z), CONFIG.lerpSpeed);
                    mesh.lookAt(camera.position);
                    if (z > maxZ) { maxZ = z; bestIdx = i; }
                    if (z > 5) {
                        const ds = 1.0 + (z / CONFIG.photoOrbitRadius) * 0.8;
                        mesh.scale.lerp(new THREE.Vector3(ds, ds, ds), CONFIG.lerpSpeed);
                    } else {
                        mesh.scale.lerp(new THREE.Vector3(0.6, 0.6, 0.6), CONFIG.lerpSpeed);
                    }
                });
                ...
                photoMeshes.forEach((mesh, i) => {
                    if (i === selectedIndex) {
                        mesh.position.lerp(new THREE.Vector3(0, 0, 60), CONFIG.lerpSpeed);
                        mesh.scale.lerp(new THREE.Vector3(5, 5, 5), CONFIG.lerpSpeed);
                        mesh.lookAt(camera.position); mesh.rotation.z = 0;
                    } else {
                        mesh.scale.lerp(new THREE.Vector3(0, 0, 0), CONFIG.lerpSpeed);
                    }
                });
            }
        ...
        function startSystem() {
            ...
            document.getElementById('hand-indicator').style.display = 'block';
            ...
            // OPTIMIZED: modelComplexity 0 for faster processing
            hands.setOptions({
                maxNumHands: 2,
                modelComplexity: 0,  // Giảm từ 1 → 0 cho nhẹ hơn
            ...
            hands.onResults(results => {
                ...
                if (results.multiHandLandmarks.length === 2) {
                    ...
                    if (distIndex < 0.15 && distThumb < 0.15) {
                        state = 'HEART';
                        statusDiv.innerText = "🫶 TẠO TIM - I LOVE YOU";
                ...
                if (results.multiHandLandmarks.length > 0) {
                    ...
                    if (avgDist < 0.25) {
                        ...
                        state = 'PHOTO';
                        statusDiv.innerText = "👌 XEM ẢNH #" + (selectedIndex + 1);
                        statusDiv.style.color = "#00FFFF";
                    } else {
                        state = 'EXPLODE';
                        statusDiv.innerText = "🖐 BUNG QUÀ - Di tay ◀▶ chọn ảnh";
